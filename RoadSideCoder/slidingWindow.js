...
// ?____________________________________________________________________________
  ...
  function printPattern(rows) {
    for (let i = 1; i <= rows; i++) {
      let leftPart = '';
      let rightPart = '';
      let middleSpaces = rows - i; // Number of middle spaces decreases with each row
      ...
      for (let k = 9; k > 9 - i; k--) {
        rightPart = k + rightPart; // Prepend to keep order
      }
  
      // Combine the left part, spaces, and right part
      let fullPattern = leftPart + ' '.repeat(middleSpaces) + rightPart;
